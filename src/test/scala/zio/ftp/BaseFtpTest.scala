...
abstract class BaseFtpTest(labelSuite: String, settings: FtpSettings, home: Path = Paths.get("ftp-home/ftp/home")) extends DefaultRunnableSpec(

  suite(labelSuite)(
    testM("invalid credentials")(
      for {
        failure <- connect(settings.copy(credentials = credentials("test", "test"))).use(_ => IO.succeed(""))
    ...
    testM("readFile does not exist") {
      for {
        invalid <- connect(settings).use(readFile("/invalid.txt")(_).chunkN(100).chunks.run(ZSink.utf8DecodeChunk))
    ...
    testM("mkdir fail when invalid path") {
     for {
       failure <- connect(settings).use(mkdir("/dir1/users.csv"))
    ...
    testM("rm valid path") {
        ...
        for {
          success <- connect(settings).use(rm("/to-delete.txt"))
    ...
    testM("rm fail when invalid path") {
      for {
        invalid <- connect(settings).use(rm("/dont-exist"))
          .foldCause( _.dieOption.fold("")(_.getMessage), _ => "")
    ...
    testM("rm fail invalid directory"){
      for {
        r <- connect(settings).use(rmdir("/dont-exist"))
          .foldCause(_.dieOption.fold("")(_.getMessage), _ => "")
    ...
    testM("upload fail when path is invalid") {
      ...
      for {
        failure <- connect(settings).use(upload("/dont-exist/hello-world.txt", data))
          .foldCause(_.dieOption.fold("")(_.getMessage), _ => "")
