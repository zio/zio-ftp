...
object SFtpTest extends DefaultRunnableSpec {
  ...
  override def spec = suite("SFtpSpec")(
    ...
    testM("ls descendant")(
      ...
      } yield assert(files.map(_.path))(hasSameElements(List("/empty.txt", "/work/notes.txt", "/work/dir1/users.csv", "/work/dir1/console.dump")))
    ),
